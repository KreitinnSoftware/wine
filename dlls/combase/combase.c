...
extern HRESULT WINAPI Ole32DllGetClassObject(REFCLSID rclsid, REFIID riid,
                                             void **obj);

/*
 * Number of times CoInitialize is called. It is decreased every time
 * CoUninitialize is called. When it hits 0, the COM libraries are freed
 */
...
static CRITICAL_SECTION_DEBUG psclsid_cs_debug = {
    ...
    0,
    0,
    {(DWORD_PTR)(__FILE__ ": cs_registered_psclsid_list")}};
...
static CRITICAL_SECTION_DEBUG registered_classes_cs_debug = {
    ...
    {&registered_classes_cs_debug.ProcessLocksList,
     &registered_classes_cs_debug.ProcessLocksList},
    0,
    0,
    {(DWORD_PTR)(__FILE__ ": registered_classes_cs")}};
static CRITICAL_SECTION registered_classes_cs = {
    &registered_classes_cs_debug, -1, 0, 0, 0, 0};

IUnknown *com_get_registered_class_object(const struct apartment *apt,
                                          REFCLSID rclsid, DWORD clscontext) {
  struct registered_class *cur;
  IUnknown *object = NULL;

  EnterCriticalSection(&registered_classes_cs);

  LIST_FOR_EACH_ENTRY(cur, &registered_classes, struct registered_class,
                      entry) {
    if ((apt->oxid == cur->apartment_id) && (clscontext & cur->clscontext) &&
...
static NTSTATUS create_key(HKEY *retkey, ACCESS_MASK access,
                           const OBJECT_ATTRIBUTES *attr) {
  NTSTATUS status =
      NtCreateKey((HANDLE *)retkey, access, attr, 0, NULL, 0, NULL);
  ...
  if (status == STATUS_OBJECT_NAME_NOT_FOUND) {
    ...
    while (i < len) {
      ...
      if (attr2.RootDirectory != attr->RootDirectory)
        NtClose(attr2.RootDirectory);
      if (status)
        return status;
...
static HKEY create_classes_root_hkey(DWORD access) {
  ...
  if (create_key(&hkey, access, &attr))
    return 0;
...
static LSTATUS create_classes_key(HKEY hkey, const WCHAR *name, REGSAM access,
                                  HKEY *retkey) {
  OBJECT_ATTRIBUTES attr;
...
static HKEY get_classes_root_hkey(HKEY hkey, REGSAM access) {
  ...
  if (hkey == HKEY_CLASSES_ROOT &&
    ...
    ret =
        create_classes_root_hkey(MAXIMUM_ALLOWED | (access & KEY_WOW64_64KEY));
...
static LSTATUS open_classes_key(HKEY hkey, const WCHAR *name, REGSAM access,
                                HKEY *retkey) {
  OBJECT_ATTRIBUTES attr;
  UNICODE_STRING nameW;
...
HRESULT open_key_for_clsid(REFCLSID clsid, const WCHAR *keyname, REGSAM access,
                           HKEY *subkey) {
...
static void com_cleanup_tlsdata(void) {
  ...
  LIST_FOR_EACH_ENTRY_SAFE(cursor, cursor2, &tlsdata->spies, struct init_spy,
                           entry) {
...
static inline struct global_options *
impl_from_IGlobalOptions(IGlobalOptions *iface) {
...
static HRESULT WINAPI global_options_QueryInterface(IGlobalOptions *iface,
  ...
  if (IsEqualGUID(&IID_IGlobalOptions, riid) ||
...
static HRESULT WINAPI global_options_Query(IGlobalOptions *iface,
  ...
  if (property < COMGLB_EXCEPTION_HANDLING ||
      property > COMGLB_PROPERTIES_RESERVED3)
    return E_INVALIDARG;
...
static const IGlobalOptionsVtbl global_options_vtbl = {
    global_options_QueryInterface, global_options_AddRef,
    global_options_Release, global_options_Set, global_options_Query};
...
static const IClassFactoryVtbl global_options_factory_vtbl = {
    class_factory_QueryInterface, class_factory_AddRef, class_factory_Release,
    global_options_CreateInstance, class_factory_LockServer};

static IClassFactory global_options_factory = {&global_options_factory_vtbl};

static HRESULT get_builtin_class_factory(REFCLSID rclsid, REFIID riid,
                                         void **obj) {
  ...
  *pvarDest = *pvarSrc;
  ...
  case VT_LPWSTR:
    if (pvarSrc->pwszVal) {
      ...
      CopyMemory(pvarDest->pwszVal, pvarSrc->pwszVal,
                 (len + 1) * sizeof(WCHAR));
    }
    break;
  case VT_BLOB:
  ...
  case VT_CF:
    if (pvarSrc->pclipdata) {
      ...
      CopyMemory(pvarDest->pclipdata->pClipData, pvarSrc->pclipdata->pClipData,
                 len);
    }
    break;
  default:
    if (pvarSrc->vt & VT_VECTOR) {
      ...
      case VT_I1:
        elemSize = sizeof(pvarSrc->cVal);
        break;
      case VT_UI1:
        elemSize = sizeof(pvarSrc->bVal);
        break;
      case VT_I2:
        elemSize = sizeof(pvarSrc->iVal);
        break;
      case VT_UI2:
        elemSize = sizeof(pvarSrc->uiVal);
        break;
      case VT_BOOL:
        elemSize = sizeof(pvarSrc->boolVal);
        break;
      case VT_I4:
        elemSize = sizeof(pvarSrc->lVal);
        break;
      case VT_UI4:
        elemSize = sizeof(pvarSrc->ulVal);
        break;
      case VT_R4:
        elemSize = sizeof(pvarSrc->fltVal);
        break;
      case VT_R8:
        elemSize = sizeof(pvarSrc->dblVal);
        break;
      case VT_ERROR:
        elemSize = sizeof(pvarSrc->scode);
        break;
      case VT_I8:
        elemSize = sizeof(pvarSrc->hVal);
        break;
      case VT_UI8:
        elemSize = sizeof(pvarSrc->uhVal);
        break;
      case VT_CY:
        elemSize = sizeof(pvarSrc->cyVal);
        break;
      case VT_DATE:
        elemSize = sizeof(pvarSrc->date);
        break;
      case VT_FILETIME:
        elemSize = sizeof(pvarSrc->filetime);
        break;
      case VT_CLSID:
        elemSize = sizeof(*pvarSrc->puuid);
        break;
      case VT_CF:
        elemSize = sizeof(*pvarSrc->pclipdata);
        break;
      case VT_BSTR:
        elemSize = sizeof(pvarSrc->bstrVal);
        break;
      case VT_LPSTR:
        elemSize = sizeof(pvarSrc->pszVal);
        break;
      case VT_LPWSTR:
        elemSize = sizeof(pvarSrc->pwszVal);
        break;
      case VT_VARIANT:
        elemSize = sizeof(*pvarSrc->pvarVal);
        break;
      ...
      if (pvarSrc->vt == (VT_VECTOR | VT_VARIANT)) {
        for (i = 0; i < len; i++)
          PropVariantCopy(&pvarDest->capropvar.pElems[i],
                          &pvarSrc->capropvar.pElems[i]);
      } else if (pvarSrc->vt == (VT_VECTOR | VT_CF)) {
        FIXME("Copy clipformats\n");
      } else if (pvarSrc->vt == (VT_VECTOR | VT_BSTR)) {
        for (i = 0; i < len; i++)
          pvarDest->cabstr.pElems[i] =
              SysAllocString(pvarSrc->cabstr.pElems[i]);
        ...
        for (i = 0; i < len; i++) {
          ...
          memcpy(pvarDest->calpstr.pElems[i], pvarSrc->calpstr.pElems[i],
                 strLen);
        }
      } else if (pvarSrc->vt == (VT_VECTOR | VT_LPWSTR)) {
        size_t strLen;
        for (i = 0; i < len; i++) {
          strLen = (lstrlenW(pvarSrc->calpwstr.pElems[i]) + 1) * sizeof(WCHAR);
          pvarDest->calpstr.pElems[i] = CoTaskMemAlloc(strLen);
          memcpy(pvarDest->calpstr.pElems[i], pvarSrc->calpstr.pElems[i],
                 strLen);
        }
      } else
        CopyMemory(pvarDest->capropvar.pElems, pvarSrc->capropvar.pElems,
                   len * elemSize);
...
HRESULT WINAPI CoQueryProxyBlanket(IUnknown *proxy, DWORD *authn_service,
                                   DWORD *authz_service, OLECHAR **servername,
  ...
  TRACE("%p, %p, %p, %p, %p, %p, %p, %p.\n", proxy, authn_service,
        authz_service, servername, authn_level, imp_level, auth_info,
        capabilities);

  hr = IUnknown_QueryInterface(proxy, &IID_IClientSecurity,
                               (void **)&client_security);
  if (SUCCEEDED(hr)) {
    hr = IClientSecurity_QueryBlanket(client_security, proxy, authn_service,
                                      authz_service, servername, authn_level,
 ...
 */
HRESULT WINAPI CoSetProxyBlanket(IUnknown *proxy, DWORD authn_service,
                                 DWORD authz_service, OLECHAR *servername,
                                 DWORD authn_level, DWORD imp_level,
                                 void *auth_info, DWORD capabilities) {
  IClientSecurity *client_security;
  ...
  hr = IUnknown_QueryInterface(proxy, &IID_IClientSecurity,
                               (void **)&client_security);
  if (SUCCEEDED(hr)) {
    hr = IClientSecurity_SetBlanket(client_security, proxy, authn_service,
                                    authz_service, servername, authn_level,
                                    imp_level, auth_info, capabilities);
    IClientSecurity_Release(client_security);
  }
 ...
 */
HRESULT WINAPI CoQueryClientBlanket(DWORD *authn_service, DWORD *authz_service,
                                    OLECHAR **servername, DWORD *authn_level,
                                    DWORD *imp_level, RPC_AUTHZ_HANDLE *privs,
                                    DWORD *capabilities) {
  IServerSecurity *server_security;
  HRESULT hr;

  TRACE("%p, %p, %p, %p, %p, %p, %p.\n", authn_service, authz_service,
        servername, authn_level, imp_level, privs, capabilities);

  hr = CoGetCallContext(&IID_IServerSecurity, (void **)&server_security);
  if (SUCCEEDED(hr)) {
    hr = IServerSecurity_QueryBlanket(server_security, authn_service,
                                      authz_service, servername, authn_level,
...
HRESULT WINAPI CoInitializeSecurity(PSECURITY_DESCRIPTOR sd, LONG cAuthSvc,
                                    ...
                                    void *reserved1, DWORD authn_level,
                                    DWORD imp_level, void *reserved2,
                                    DWORD capabilities, void *reserved3) {
  FIXME("%p, %ld, %p, %p, %ld, %ld, %p, %ld, %p stub\n", sd, cAuthSvc,
        asAuthSvc, reserved1, authn_level, imp_level, reserved2, capabilities,
        reserved3);
...
HRESULT WINAPI DECLSPEC_HOTPATCH ProgIDFromCLSID(REFCLSID clsid,
  ...
  HKEY hkey;
  REGSAM opposite =
      (sizeof(void *) > sizeof(int)) ? KEY_WOW64_32KEY : KEY_WOW64_64KEY;
  BOOL is_wow64;
  HRESULT hr;
  ...
  *progid = NULL;
  ...
  if (FindActCtxSectionGuid(0, NULL,
    ...
    struct comclassredirect_data *comclass =
        (struct comclassredirect_data *)data.lpData;
...
static BOOL guid_from_string(LPCWSTR s, GUID *id) {
  ...
  for (i = 20; i < 37; i += 2) {
    ...
    if (!is_valid_hex(s[i]) || !is_valid_hex(s[i + 1]))
      return FALSE;
    id->Data4[(i - 20) / 2] =
        guid_conv_table[s[i]] << 4 | guid_conv_table[s[i + 1]];
  }
...
HRESULT WINAPI DECLSPEC_HOTPATCH CLSIDFromProgID(LPCOLESTR progid,
  ...
  if (FindActCtxSectionStringW(
          0, NULL, ACTIVATION_CONTEXT_SECTION_COM_PROGID_REDIRECTION, progid,
          &data)) {
    struct progidredirect_data *progiddata =
        (struct progidredirect_data *)data.lpData;
    CLSID *alias =
        (CLSID *)((BYTE *)data.lpSectionBase + progiddata->clsid_offset);
    *clsid = *alias;
    return S_OK;
  }
...
HRESULT WINAPI StringFromCLSID(REFCLSID clsid, LPOLESTR *str) {
  if (!(*str = CoTaskMemAlloc(CHARS_IN_GUID * sizeof(WCHAR))))
    return E_OUTOFMEMORY;
...
INT WINAPI StringFromGUID2(REFGUID guid, LPOLESTR str, INT cmax) {
  if (!guid || cmax < CHARS_IN_GUID)
    return 0;
  swprintf(str, CHARS_IN_GUID,
...
static HRESULT return_multi_qi(IUnknown *unk, DWORD count, MULTI_QI *mqi,
  ...
  for (; index < count; index++) {
    mqi[index].hr = IUnknown_QueryInterface(unk, mqi[index].pIID,
                                            (void **)&mqi[index].pItf);
...
CoGetInstanceFromFile(COSERVERINFO *server_info, CLSID *rclsid, IUnknown *outer,
                      DWORD cls_context, DWORD grfmode, OLECHAR *filename,
  ...
  hr = CoCreateInstance(rclsid, outer, cls_context, &IID_IUnknown,
                        (void **)&obj);
  if (hr != S_OK) {
...
HRESULT WINAPI CoGetInstanceFromIStorage(COSERVERINFO *server_info,
                                         CLSID *rclsid, IUnknown *outer,
                                         DWORD cls_context, IStorage *storage,
                                         DWORD count, MULTI_QI *results) {
...
HRESULT WINAPI DECLSPEC_HOTPATCH CoCreateInstance(REFCLSID rclsid,
  ...
  TRACE("%s, %p, %#lx, %s, %p.\n", debugstr_guid(rclsid), outer, cls_context,
 ...
 */
HRESULT WINAPI CoCreateInstanceFromApp(REFCLSID rclsid, IUnknown *outer,
                                       DWORD cls_context, void *server_info,
                                       ULONG count, MULTI_QI *results) {
  TRACE("%s, %p, %#lx, %p, %lu, %p\n", debugstr_guid(rclsid), outer,
        cls_context, server_info, count, results);

  return CoCreateInstanceEx(rclsid, outer, cls_context | CLSCTX_APPCONTAINER,
                            server_info, count, results);
}

static HRESULT com_get_class_object(REFCLSID rclsid, DWORD clscontext,
                                    COSERVERINFO *server_info, REFIID riid,
                                    void **obj) {
  ...
  *obj = NULL;
  ...
  if (!(apt = apartment_get_current_or_mta())) {
    WARN("apartment not initialised, implicitly initializing as STA\n");
  ...
  if (clscontext & CLSCTX_INPROC_SERVER) {
    if (IsEqualCLSID(rclsid, &CLSID_InProcFreeMarshaler) ||
        ...
        (!(clscontext & CLSCTX_APPCONTAINER) &&
  ...
  if (clscontext & CLSCTX_INPROC) {
    ...
    if (FindActCtxSectionGuid(FIND_ACTCTX_SECTION_KEY_RETURN_HACTCTX, NULL,
      ...
      clsreg.u.actctx.module_name =
          (WCHAR *)((BYTE *)data.lpSectionBase + comclass->name_offset);
      clsreg.u.actctx.hactctx = data.hActCtx;
      clsreg.u.actctx.threading_model = comclass->model;
      clsreg.origin = CLASS_REG_ACTCTX;

      hr = apartment_get_inproc_class_object(apt, &clsreg, &comclass->clsid,
                                             riid, clscontext, obj);
  ...
  if (!(clscontext & CLSCTX_APPCONTAINER) &&
      (registered_obj =
           com_get_registered_class_object(apt, rclsid, clscontext))) {
  ...
  if (clscontext & CLSCTX_INPROC_SERVER) {
    ...
    if (SUCCEEDED(hr)) {
      ...
      hr = apartment_get_inproc_class_object(apt, &clsreg, rclsid, riid,
                                             clscontext, obj);
      RegCloseKey(hkey);
    }
 ...
 */
HRESULT WINAPI DECLSPEC_HOTPATCH
CoCreateInstanceEx(REFCLSID rclsid, IUnknown *outer, DWORD cls_context,
  ...
  if (FAILED(hr = com_get_class_object(&clsid, cls_context, NULL,
                                       &IID_IClassFactory, (void **)&factory)))
    return hr;

  hr = IClassFactory_CreateInstance(factory, outer, results[0].pIID,
                                    (void **)&unk);
...
HRESULT WINAPI CoRevokeInitializeSpy(ULARGE_INTEGER cookie) {
  ...
  if (!(spy = get_spy_entry(tlsdata, cookie.u.LowPart)))
    return E_INVALIDARG;
...
static BOOL com_peek_message(struct apartment *apt, MSG *msg) {
  /* First try to retrieve messages for incoming COM calls to the apartment
   * window */
  return (apt->win &&
          PeekMessageW(msg, apt->win, 0, 0, PM_REMOVE | PM_NOYIELD)) ||
         /* Next retrieve other messages necessary for the app to remain
            responsive */
         PeekMessageW(msg, NULL, WM_DDE_FIRST, WM_DDE_LAST,
                      PM_REMOVE | PM_NOYIELD) ||
         PeekMessageW(msg, NULL, 0, 0,
                      PM_QS_PAINT | PM_QS_SENDMESSAGE | PM_REMOVE | PM_NOYIELD);
}

/***********************************************************************
 *           CoWaitForMultipleHandles    (combase.@)
 */
HRESULT WINAPI CoWaitForMultipleHandles(DWORD flags, DWORD timeout,
                                        ULONG handle_count, HANDLE *handles,
                                        DWORD *index) {
  ...
  TRACE("%#lx, %#lx, %lu, %p, %p\n", flags, timeout, handle_count, handles,
        index);
  ...
  while (TRUE) {
    ...
    if (message_loop) {
      ...
      if (res == WAIT_OBJECT_0 + handle_count) /* messages available */
      {
        ...
        if (apt->filter) {
          PENDINGTYPE pendingtype = tlsdata->pending_call_count_server
                                        ? PENDINGTYPE_NESTED
                                        : PENDINGTYPE_TOPLEVEL;
          DWORD be_handled = IMessageFilter_MessagePending(
              apt->filter, 0 /* FIXME */, now - start_time, pendingtype);
        ...
        if (!apt->win) {
          /* If window is NULL on apartment, peek at messages so that it will
           * not trigger MsgWaitForMultipleObjects next time. */
          PeekMessageW(NULL, NULL, 0, 0,
                       PM_QS_POSTMESSAGE | PM_NOREMOVE | PM_NOYIELD);
        }

        /* Some apps (e.g. Visio 2010) don't handle WM_PAINT properly and loop
         * forever, so after processing 100 messages we go back to checking the
         * wait handles */
      ...
      res = WaitForMultipleObjectsEx(
          handle_count, handles, !!(flags & COWAIT_WAITALL),
          (timeout == INFINITE) ? INFINITE : start_time + timeout - now,
...
static void com_revoke_all_ps_clsids(void) {
  ...
  LIST_FOR_EACH_ENTRY_SAFE(cur, cur2, &registered_proxystubs,
                           struct registered_ps, entry) {
...
static HRESULT get_ps_clsid_from_registry(const WCHAR *path, REGSAM access,
                                          CLSID *pclsid) {
  ...
  len = sizeof(value);
  if (ERROR_SUCCESS !=
      RegQueryValueExW(hkey, NULL, NULL, NULL, (BYTE *)value, &len))
    return REGDB_E_IIDNOTREG;
  RegCloseKey(hkey);
...
HRESULT WINAPI CoGetPSClsid(REFIID riid, CLSID *pclsid) {
  ...
  LIST_FOR_EACH_ENTRY(cur, &registered_proxystubs, struct registered_ps,
                      entry) {
  ...
  if (FindActCtxSectionGuid(
          0, NULL, ACTIVATION_CONTEXT_SECTION_COM_INTERFACE_REDIRECTION, riid,
          &data)) {
    struct ifacepsredirect_data *ifaceps =
        (struct ifacepsredirect_data *)data.lpData;
    *pclsid = ifaceps->iid;
    return S_OK;
  }
...
impl_from_IComThreadingInfo(IComThreadingInfo *iface) {
  return CONTAINING_RECORD(iface, struct thread_context,
                           IComThreadingInfo_iface);
}

static inline struct thread_context *
impl_from_IContextCallback(IContextCallback *iface) {
  return CONTAINING_RECORD(iface, struct thread_context,
...
static HRESULT WINAPI thread_context_info_QueryInterface(
    IComThreadingInfo *iface, REFIID riid, void **obj) {
...
static ULONG WINAPI thread_context_info_Release(IComThreadingInfo *iface) {
  ...
  /* Context instance is initially created with CoGetContextToken() with
     refcount set to 0, releasing context while refcount is at 0 destroys it. */
  if (!context->refcount) {
    free(context);
    return 0;
  }
...
static HRESULT WINAPI thread_context_info_GetCurrentApartmentType(
    IComThreadingInfo *iface, APTTYPE *apttype) {
...
static HRESULT WINAPI thread_context_info_GetCurrentThreadType(
    IComThreadingInfo *iface, THDTYPE *thdtype) {
  APTTYPEQUALIFIER qualifier;
  APTTYPE apttype;
  HRESULT hr;
...
static HRESULT WINAPI thread_context_info_GetCurrentLogicalThreadId(
    IComThreadingInfo *iface, GUID *logical_thread_id) {
  TRACE("%p\n", logical_thread_id);

  return CoGetCurrentLogicalThreadId(logical_thread_id);
}

static HRESULT WINAPI thread_context_info_SetCurrentLogicalThreadId(
...
static HRESULT WINAPI thread_context_callback_QueryInterface(
    IContextCallback *iface, REFIID riid, void **obj) {
  struct thread_context *context = impl_from_IContextCallback(iface);
  return IComThreadingInfo_QueryInterface(&context->IComThreadingInfo_iface,
                                          riid, obj);
}
...
static HRESULT WINAPI thread_context_callback_ContextCallback(
    IContextCallback *iface, PFNCONTEXTCALL callback, ComCallData *param,
    REFIID riid, int method, IUnknown *punk) {
  FIXME("%p, %p, %p, %s, %d, %p\n", iface, callback, param, debugstr_guid(riid),
        method, punk);
...
static const IContextCallbackVtbl thread_context_callback_vtbl = {
    ...
    thread_context_callback_Release, thread_context_callback_ContextCallback};
...
static HRESULT WINAPI thread_object_context_EnumContextProps(
    IObjContext *iface, IEnumContextProps **props) {
  FIXME("%p, %p\n", iface, props);
...
static const IObjContextVtbl thread_object_context_vtbl = {
    thread_object_context_QueryInterface,   thread_object_context_AddRef,
    thread_object_context_Release,          thread_object_context_SetProperty,
    thread_object_context_RemoveProperty,   thread_object_context_GetProperty,
    thread_object_context_EnumContextProps, thread_object_context_Reserved1,
    thread_object_context_Reserved2,        thread_object_context_Reserved3,
    thread_object_context_Reserved4,        thread_object_context_Reserved5,
    thread_object_context_Reserved6,        thread_object_context_Reserved7};
...
HRESULT WINAPI DECLSPEC_HOTPATCH CoInitializeEx(void *reserved, DWORD model) {
  ...
  LIST_FOR_EACH_ENTRY(cursor, &tlsdata->spies, struct init_spy, entry) {
    if (cursor->spy)
      IInitializeSpy_PreInitialize(cursor->spy, model, tlsdata->inits);
  }
  ...
  LIST_FOR_EACH_ENTRY(cursor, &tlsdata->spies, struct init_spy, entry) {
    if (cursor->spy)
      hr =
          IInitializeSpy_PostInitialize(cursor->spy, hr, model, tlsdata->inits);
  }
  unlock_init_spies(tlsdata);
...
void WINAPI DECLSPEC_HOTPATCH CoUninitialize(void) {
  ...
  lock_init_spies(tlsdata);
  LIST_FOR_EACH_ENTRY_SAFE(cursor, next, &tlsdata->spies, struct init_spy,
                           entry) {
    if (cursor->spy)
      IInitializeSpy_PreUninitialize(cursor->spy, tlsdata->inits);
  }
  unlock_init_spies(tlsdata);
  ...
  if (!tlsdata->inits) {
    ...
    LIST_FOR_EACH_ENTRY_SAFE(cursor, next, &tlsdata->spies, struct init_spy,
                             entry) {
      if (cursor->spy)
        IInitializeSpy_PostUninitialize(cursor->spy, tlsdata->inits);
    }
    unlock_init_spies(tlsdata);
 ...
 */
HRESULT WINAPI CoRegisterClassObject(REFCLSID rclsid, IUnknown *object,
                                     DWORD clscontext, DWORD flags,
                                     DWORD *cookie) {
  ...
  TRACE("%s, %p, %#lx, %#lx, %p\n", debugstr_guid(rclsid), object, clscontext,
        flags, cookie);
  ...
  *cookie = newclass->cookie;
  ...
  if (clscontext & CLSCTX_LOCAL_SERVER) {
    ...
    hr = rpc_register_local_server(&newclass->clsid, marshal_stream, flags,
                                   &newclass->rpcss_cookie);
...
static void com_revoke_local_servers(void) {
  ...
  LIST_FOR_EACH_ENTRY_SAFE(cur, cur2, &registered_classes,
 ...
 */
HRESULT WINAPI CoLockObjectExternal(IUnknown *object, BOOL lock,
